...
su - $VAGRANT_USER -c "cp -f /vagrant/vagrant/templates/env.example ~/.env"
dos2unix $VAGRANT_HOME/.env
...
apt-get install -y \
    $(: System packages and tools) \
        build-essential mc git git-flow dos2unix bash-completion libssl-dev \
    $(: Python dependencies) \
